#include <vigir_footstep_planning_default_plugins/state_generator/custom_reachability_state_generator.h>
...
{
    CustomReachabilityStateGenerator::CustomReachabilityStateGenerator()
            : StateGeneratorPlugin("custom_reachability_state_generator")
    {
    }

    bool CustomReachabilityStateGenerator::loadParams(const vigir_generic_params::ParameterSet& params)
    {
      ...
      ROS_ERROR("START OF CUSTOM_REACHABILITY_STATE_GENERATOR.CPP");
      ...
      for (int y = ivMaxInvStepRangeY; y <= ivMaxStepRangeY; y++)
      {
        for (int x = ivMaxInvStepRangeX; x <= ivMaxStepRangeX; x++)
        {
          ...
          if (!in_step_range)
            continue;
          double k [6] = {0.03, 1 * cell_size, 10 *cell_size, 1*cell_size, 10*cell_size ,1*cell_size};
...
//          double delta_theta = 0.0 - angle_cell_2_state(theta, angle_bin_size);
            double delta_x = x;
            double delta_y = y - disc_val(foot_separation, cell_size);
            ...
            if (delta_x > 0) {
              ...
              cost += k[2] * std::abs(delta_x);
              ROS_INFO("k[1] calc val: %f", cost);
            }
            double k2 = k[3] * std::abs(delta_y);
            double k3 = k[4] * std::min(1.0, std::max(0.0, form_a)) * std::abs(delta_y);
            double k4 = k[5] * std::abs(delta_theta);
...
//            Footstep f(cont_val(x, cell_size), cont_val(y, cell_size), angle_cell_2_state(theta, angle_bin_size), 0.0, cell_size, num_angle_bins, hash_table_size);
            ...
            ROS_INFO("Delta x as cont val: %f", cont_val(delta_x, cell_size));
            ROS_INFO("Delta y as cont val: %f", cont_val(delta_y, cell_size));
            ROS_INFO("Delta theta as cont val: %f", angle_cell_2_state(delta_theta, angle_bin_size));
            ROS_WARN("Cost of current footstep: %f", cost);
            // dump to screen
          }
        }
      }
      ROS_ERROR("END OF CUSTOM_REACHABILITY_STATE_GENERATOR.CPP");
      return result;
    }
...
PLUGINLIB_EXPORT_CLASS(vigir_footstep_planning::CustomReachabilityStateGenerator, vigir_footstep_planning::StateGeneratorPlugin)
